function mass_area = Function_Area_SA(x)
...
for i=1:num_rows_conn
    if abs(sigma(i, 1)) >= 276e6
        g = abs(sigma(i, 1)) - 276e6;
        mass_area = mass_area + rho * new_L(i, 1) * x(i) + r*max(0, g/276e6)^2;
    else
        mass_area = mass_area + rho * new_L(i, 1) * x(i);
    end
end
end