...
x = [0.47807     0.41069     0.67825     0.32995     0.48096     0.21624];
...
r = 1000;
for i=1:num_rows_conn
    if abs(sigma(i, 1)) >= 276e6
        g1 = abs(sigma(i, 1)) - 276e6;
        mass_shape = mass_shape + rho * new_L(i, 1) * A + r*max(0, g1/276e6)^2;
    else
...
for i = 1:size(conn, 1)
    node1 = conn(i, 1);
    node2 = conn(i, 2);
    line([coord(node1, 1), coord(node2, 1)], [coord(node1, 2), coord(node2, 2)], 'Color', 'k');
end

hold on;
scatter(coord(:, 1), coord(:, 2), 'filled');
text(coord(:, 1), coord(:, 2), num2str((1:numNodes)'),'VerticalAlignment','bottom','HorizontalAlignment','right');
hold off;
